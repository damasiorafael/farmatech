...
$templatefile = ($inlineedit == EDIT_INLINE) ? "servicos_inline_edit.htm" : "servicos_edit.htm";
...
//	processing titulo - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$value = postvalue("value_titulo_".$id);
		$type = postvalue("type_titulo_".$id);
		if(FieldSubmitted("titulo_".$id))
		{
		...
		{	
			...
			if(0 && "titulo"=="senha" && $url_page=="admin_users_")
				$value = md5($value);
...
//	processing titulo - end
//	processing img_destaque - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$value = postvalue("value_img_destaque_".$id);
		$type = postvalue("type_img_destaque_".$id);
		if(FieldSubmitted("img_destaque_".$id))
		{
			$fileNameForPrepareFunc = postvalue("filename_img_destaque_".$id);
					if($fileNameForPrepareFunc)
				$value = $fileNameForPrepareFunc;
			if(substr($type,0,4)=="file")
				$value = prepare_file($value,"img_destaque",$type,$fileNameForPrepareFunc ,$id);
...
// file deletion, read filename from the database
					...
					$fileNameForPrepareFunc = $dataold["img_destaque"];
				}
				$value = prepare_upload("img_destaque",$type,$fileNameForPrepareFunc,$value,"" ,$id,$pageObject);
			}
		}
		else
			$value = false;
		...
		{	
				if($value)
				$contents = GetUploadedFileContents("value_img_destaque_".$id);

						if($value)
			{
				$ext = CheckImageExtension(GetUploadedFileName("value_img_destaque_".$id));
				$thumb = CreateThumbnail($contents,150,$ext);
				$pageObject->filesToSave[] = new SaveFile($thumb,"th".$value,GetUploadFolder("img_destaque"), GetFieldData("","img_destaque","Absolute",false));
			}

	
	
	
			if(0 && "img_destaque"=="senha" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["img_destaque"] = $value;
		
			}
	
		}
//	processing img_destaque - end
//	processing img_interna - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$value = postvalue("value_img_interna_".$id);
		$type = postvalue("type_img_interna_".$id);
		if(FieldSubmitted("img_interna_".$id))
		{
			$fileNameForPrepareFunc = postvalue("filename_img_interna_".$id);
					if($fileNameForPrepareFunc)
				$value = $fileNameForPrepareFunc;
			if(substr($type,0,4)=="file")
				$value = prepare_file($value,"img_interna",$type,$fileNameForPrepareFunc ,$id);
			else if(substr($type,0,6)=="upload")
			{
				if($type=="upload1")
				{
// file deletion, read filename from the database
					...
					$fileNameForPrepareFunc = $dataold["img_interna"];
				}
				$value = prepare_upload("img_interna",$type,$fileNameForPrepareFunc,$value,"" ,$id,$pageObject);
			}
		}
		else
			$value = false;
		...
		{	
				if($value)
				$contents = GetUploadedFileContents("value_img_interna_".$id);

						if($value)
			{
				$ext = CheckImageExtension(GetUploadedFileName("value_img_interna_".$id));
				$thumb = CreateThumbnail($contents,150,$ext);
				$pageObject->filesToSave[] = new SaveFile($thumb,"th".$value,GetUploadFolder("img_interna"), GetFieldData("","img_interna","Absolute",false));
			}

	
	
	
			if(0 && "img_interna"=="senha" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["img_interna"] = $value;
		
			}
	
		}
//	processing img_interna - end
//	processing conteudo - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode
	...
	if($condition)
	{
		...
		{	
			...
			if(0 && "conteudo"=="senha" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["conteudo"] = $value;
		
			}
	
		}
//	processing conteudo - end
//	processing destaque_home - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$value = postvalue("value_destaque_home_".$id);
		$type = postvalue("type_destaque_home_".$id);
		if(FieldSubmitted("destaque_home_".$id))
		{
				$value = prepare_for_db("destaque_home",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "destaque_home"=="senha" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["destaque_home"] = $value;
		
			}
	
		}
//	processing destaque_home - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: servicos_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: servicos_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	...
	$data["conteudo"] = $evalues["conteudo"];
	$data["destaque_home"] = $evalues["destaque_home"];
...
//	titulo - 
	...
	$showValues["titulo"] = $value;
	$showFields[] = "titulo";
		$showRawValues["titulo"] = substr($data["titulo"],0,100);

//	img_destaque - File-based Image

		$value = "";
				if(CheckImageExtension($data["img_destaque"])) 
		{
						 	// show thumbnail
			$thumbname = "th".$data["img_destaque"];
			if(substr("uploads/",0,7)!="http://" && !myfile_exists(getabspath("uploads/".$thumbname)))
				$thumbname = $data["img_destaque"];
			$value = "<a"; 
							$value.= " rel='ibox'";
			$value.= " href=\"".htmlspecialchars(AddLinkPrefix("img_destaque",$data["img_destaque"]))."\">";
			$value.= "<img";
			if(isEnableSection508())
				$value.= " alt=\"".htmlspecialchars($data["img_destaque"])."\"";
			if($thumbname==$data["img_destaque"])
			{
											}
			$value.= " id=\"img_img_destaque_".$id."\" border=0";
			$value.= " src=\"".htmlspecialchars(AddLinkPrefix("img_destaque",$thumbname))."\"></a>";
		}
	$showValues["img_destaque"] = $value;
	$showFields[] = "img_destaque";
		$showRawValues["img_destaque"] = substr($data["img_destaque"],0,100);

//	img_interna - File-based Image

		$value = "";
				if(CheckImageExtension($data["img_interna"])) 
		{
						 	// show thumbnail
			$thumbname = "th".$data["img_interna"];
			if(substr("uploads/",0,7)!="http://" && !myfile_exists(getabspath("uploads/".$thumbname)))
				$thumbname = $data["img_interna"];
			$value = "<a"; 
							$value.= " rel='ibox'";
			$value.= " href=\"".htmlspecialchars(AddLinkPrefix("img_interna",$data["img_interna"]))."\">";
			$value.= "<img";
			if(isEnableSection508())
				$value.= " alt=\"".htmlspecialchars($data["img_interna"])."\"";
			if($thumbname==$data["img_interna"])
			{
											}
			$value.= " id=\"img_img_interna_".$id."\" border=0";
			$value.= " src=\"".htmlspecialchars(AddLinkPrefix("img_interna",$thumbname))."\"></a>";
		}
	$showValues["img_interna"] = $value;
	$showFields[] = "img_interna";
		$showRawValues["img_interna"] = substr($data["img_interna"],0,100);
...
//	destaque_home - Checkbox

		$value = "";
			$value = GetData($data,"destaque_home", "Checkbox");
	$showValues["destaque_home"] = $value;
	$showFields[] = "destaque_home";
		$showRawValues["destaque_home"] = substr($data["destaque_home"],0,100);
