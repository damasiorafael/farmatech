...
$mode=SEARCH_SIMPLE;
$templatefile = "area_restrita_search.htm";

// for ajax query, used when page buffers new control
if(postvalue("mode")=="inlineLoadCtrl"){
	$mode = SEARCH_LOAD_CONTROL;
	$templatefile = "area_restrita_inline_search.htm";
}	
...
if ($mode==SEARCH_SIMPLE)
{
	...
	if(GetLookupTable("arquivo", $strTableName))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][GetLookupTable("arquivo", $strTableName)] = GetTableURL(GetLookupTable("arquivo", $strTableName));
	
	$pageObject->fillFieldToolTips("arquivo");	
		
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("arquivo");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "arquivo";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("arquivo_label","<label for=\"".GetInputElementId("arquivo", $id)."\">","</label>");
	else 
		$xt->assign("arquivo_label", true);
	
	$xt->assign("arquivo_fieldblock", true);		
	$xt->assignbyref("arquivo_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("arquivo_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("arquivo_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_arquivo", $ctrlBlockArr['searchtype']);	
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("arquivo");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl) 
	{				
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"arquivo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{	
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"arquivo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
	
	if(GetLookupTable("exclusivo", $strTableName))
		$pageObject->settingsMap["globalSettings"]['shortTNames'][GetLookupTable("exclusivo", $strTableName)] = GetTableURL(GetLookupTable("exclusivo", $strTableName));
	
	$pageObject->fillFieldToolTips("exclusivo");	
		
	$srchFields = $pageObject->searchClauseObj->getSearchCtrlParams("exclusivo");
	$firstFieldParams = array();
	if (count($srchFields))
	{
		$firstFieldParams = $srchFields[0];
	...
	{
		$firstFieldParams['fName'] = "exclusivo";
		$firstFieldParams['eType'] = '';
		$firstFieldParams['value1'] = '';
		$firstFieldParams['opt'] = '';
		$firstFieldParams['value2'] = '';
	...
	if(isEnableSection508())
		$xt->assign_section("exclusivo_label","<label for=\"".GetInputElementId("exclusivo", $id)."\">","</label>");
	else 
		$xt->assign("exclusivo_label", true);
	
	$xt->assign("exclusivo_fieldblock", true);		
	$xt->assignbyref("exclusivo_editcontrol", $ctrlBlockArr['searchcontrol']);					
	$xt->assign("exclusivo_notbox", $ctrlBlockArr['notbox']);		
	// create second control, if need it		
	$xt->assignbyref("exclusivo_editcontrol1", $ctrlBlockArr['searchcontrol1']);		
	// create search type select
	$xt->assign("searchtype_exclusivo", $ctrlBlockArr['searchtype']);	
	$isFieldNeedSecCtrl = $searchControlBuilder->isNeedSecondCtrl("exclusivo");
	$ctrlInd = 0;
	if ($isFieldNeedSecCtrl) 
	{				
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"exclusivo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd, 1=>($ctrlInd+1)));
		$ctrlInd+=2;
	}
	else
	{	
		$pageObject->controlsMap["search"]["searchBlocks"][] = array('fName'=>"exclusivo", 'recId'=>$id, 'ctrlsMap'=>array(0=>$ctrlInd));			
		$ctrlInd++;
	}
	// search fields data
