...
if($inlineadd == ADD_INLINE)
	$templatefile = "banner_home_inline_add.htm";
else
	$templatefile = "banner_home_add.htm";
...
if(@$_POST["a"]=="added")
{
	...
	$avalues=array();
	$blobfields=array();
//	processing titulo_banner - start
	...
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["titulo_banner"]=$value;
		}
		}
//	processibng titulo_banner - end
//	processing imagem - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		...
		if (FieldSubmitted("imagem_".$id))
		{
				$fileNameForPrepareFunc = postvalue("filename_imagem_".$id);
					$value=prepare_upload("imagem","upload2",$fileNameForPrepareFunc,$fileNameForPrepareFunc,"" ,$id,$pageObject);
		}
		else
			$value=false;
		...
		//	create thumbnail
			if($value)
			{
				...
				$value=md5($value);
			$avalues["imagem"]=$value;
		}
		}
//	processibng imagem - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'banner_home_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'banner_home_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: banner_home_".$pageObject->getPageType().".php");
