...
$templatefile = ($inlineedit == EDIT_INLINE) ? "banner_home_inline_edit.htm" : "banner_home_edit.htm";
...
//	processing titulo_banner - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$value = postvalue("value_titulo_banner_".$id);
		$type = postvalue("type_titulo_banner_".$id);
		if(FieldSubmitted("titulo_banner_".$id))
		{
				$value = prepare_for_db("titulo_banner",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "titulo_banner"=="senha" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["titulo_banner"] = $value;
		
			}
	
		}
//	processing titulo_banner - end
//	processing imagem - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$value = postvalue("value_imagem_".$id);
		$type = postvalue("type_imagem_".$id);
		if(FieldSubmitted("imagem_".$id))
		{
			$fileNameForPrepareFunc = postvalue("filename_imagem_".$id);
					if($fileNameForPrepareFunc)
				$value = $fileNameForPrepareFunc;
			if(substr($type,0,4)=="file")
				$value = prepare_file($value,"imagem",$type,$fileNameForPrepareFunc ,$id);
...
// file deletion, read filename from the database
					...
					$fileNameForPrepareFunc = $dataold["imagem"];
				}
				$value = prepare_upload("imagem",$type,$fileNameForPrepareFunc,$value,"" ,$id,$pageObject);
			}
		}
		else
			$value = false;
		...
		{	
				if($value)
				$contents = GetUploadedFileContents("value_imagem_".$id);

						if($value)
			{
				$ext = CheckImageExtension(GetUploadedFileName("value_imagem_".$id));
				$thumb = CreateThumbnail($contents,120,$ext);
				$pageObject->filesToSave[] = new SaveFile($thumb,"th".$value,GetUploadFolder("imagem"), GetFieldData("","imagem","Absolute",false));
			}

	
	
	
			if(0 && "imagem"=="senha" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["imagem"] = $value;
		
			}
	
		}
//	processing imagem - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: banner_home_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_banner_home->Copy();
...
if(!$data)
{
	...
	{
		header("Location: banner_home_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["titulo_banner"] = $evalues["titulo_banner"];
...
//	titulo_banner - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"titulo_banner", ""),"field=titulo%5Fbanner".$keylink,"",MODE_LIST);
	$showValues["titulo_banner"] = $value;
	$showFields[] = "titulo_banner";
		$showRawValues["titulo_banner"] = substr($data["titulo_banner"],0,100);

//	imagem - File-based Image

		$value = "";
				if(CheckImageExtension($data["imagem"])) 
		{
						 	// show thumbnail
			$thumbname = "th".$data["imagem"];
			if(substr("uploads/",0,7)!="http://" && !myfile_exists(getabspath("uploads/".$thumbname)))
				$thumbname = $data["imagem"];
			$value = "<a"; 
							$value.= " rel='ibox'";
			$value.= " href=\"".htmlspecialchars(AddLinkPrefix("imagem",$data["imagem"]))."\">";
			$value.= "<img";
			if(isEnableSection508())
				$value.= " alt=\"".htmlspecialchars($data["imagem"])."\"";
			if($thumbname==$data["imagem"])
			{
											}
			$value.= " id=\"img_imagem_".$id."\" border=0";
			$value.= " src=\"".htmlspecialchars(AddLinkPrefix("imagem",$thumbname))."\"></a>";
		}
	$showValues["imagem"] = $value;
	$showFields[] = "imagem";
		$showRawValues["imagem"] = substr($data["imagem"],0,100);
