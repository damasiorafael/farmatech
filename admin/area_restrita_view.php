...
$arr = array();
$arr['fName'] = "arquivo";
$arr['viewFormat'] = ViewFormat("arquivo", $strTableName);
$fieldsArr[] = $arr;
$arr = array();
$arr['fName'] = "usuario";
...
//arquivo - Document Download
	...
	$xt->assign("arquivo_value",$value);
	if(!$pageObject->isAppearOnTabs("arquivo"))
		$xt->assign("arquivo_fieldblock",true);
	else
		$xt->assign("arquivo_tabfieldblock",true);
////////////////////////////////////////////
//usuario - 
	
	$value="";
	$value=DisplayLookupWizard("usuario",$data["usuario"],$data,$keylink,MODE_VIEW);
...
//Begin show Next Prev button
	...
	if(count($next))
    {
		...
		$xt->assign("nextbutton_attrs","id=\"nextButton".$id."\" onclick=\"window.location.href='area_restrita_view.php?".$nextlink."'\"");
	}
	else 
	...
	if(count($prev))
	{
		...
		$xt->assign("prevbutton_attrs","id=\"prevButton".$id."\" onclick=\"window.location.href='area_restrita_view.php?".$prevlink."'\"");
	}
    else 
...
$templatefile = "area_restrita_view.htm";
