...
class RightsPage extends ListPage  
{
	...
	function getRights() 
	{
		$trs = db_query("select GroupID,TableName,AccessMask from `farmatech_ugrights` order by GroupID", $this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			foreach(postvalue("table") as $table)
			{
				...
				$sql="delete from `farmatech_ugrights` where TableName='".db_addslashes($table)."'";
				...
				foreach($this->groupsArr as $grp)
				{
					...
					if(@$_POST["cbadm_".$gtable."_".$grp[0]])
						$mask.="M";
					if($mask)
						db_exec("insert into `farmatech_ugrights` (TableName,GroupID,AccessMask) values ('".db_addslashes($table)."',".$grp[0].",'".$mask."')",$this->conn);
				}
			}
		}
	}
