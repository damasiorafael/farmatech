...
{	
	...
	{	
		...
		$this->xt->assign("addgroup_attrs","onclick=\"\$('#addarea').show();\$('#groupname')[0].focus(); \$('#groupname').val(makename('"."novo grupo"."'));\$('#gmessage').html(TEXT_AA_ADD_NEW_GROUP);renameidx=-1;\"");
	...
	function save() 
	{
		if(postvalue("a")=="save")
		{
			...
			foreach(postvalue("username") as $user)
			{
				...
				$sql="delete from `farmatech_ugmembers` where UserName='".db_addslashes($user)."'";
				...
				if(count($_POST["cb_".$useridx]))
				{
					...
					foreach(@$_POST["cb_".$useridx] as $g)
					{
						if($g<0)
						{
							if($user==$_SESSION["UserID"] && $g==-1)
								continue;
							$sql="insert into `farmatech_ugmembers` (UserName,GroupID) values ('".htmlspecialchars($user)."',".$g.")";
							db_exec($sql,$this->conn);
						}
						else
						{
					...
					if($glist!="")
					{
						$sql="insert into `farmatech_ugmembers` (UserName,GroupID) select '".htmlspecialchars($user)."',GroupID from `farmatech_uggroups` where GroupID in (".$glist.")";
						db_exec($sql,$this->conn);
					}
				}
			}
	...
	{							
		...
		while($data && ($this->sortByGroup !== false || $this->recNo <= $this->pageSize || $this->pageSize==-1))
		{
			...
			$usercheckbox_attrs.=" name=\"user_".htmlspecialchars($data["usuario"])."\"";
	...
	{
		//	select members list		
		$trs = db_query("select UserName,GroupID from `farmatech_ugmembers` order by UserName,GroupID",$this->conn);
	...
	function fillGroups() 
	{
		...
		$trs = db_query("select GroupID,Label from `farmatech_uggroups` order by Label",$this->conn);
		while($tdata = db_fetch_numarray($trs))
		{
