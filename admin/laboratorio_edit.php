...
$templatefile = ($inlineedit == EDIT_INLINE) ? "laboratorio_inline_edit.htm" : "laboratorio_edit.htm";
...
// file deletion, read filename from the database
					...
					$fileNameForPrepareFunc = $dataold["foto"];
				}
				$value = prepare_upload("foto",$type,$fileNameForPrepareFunc,$value,"" ,$id,$pageObject);
			}
		}
		else
			$value = false;
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: laboratorio_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_laboratorio->Copy();
...
if(!$data)
{
	...
	{
		header("Location: laboratorio_list.php?a=return");
		exit();
	}
	else
		$data = array();
