...
$templatefile = ($inlineedit == EDIT_INLINE) ? "orcamento_inline_edit.htm" : "orcamento_edit.htm";
...
//	processing respondido - begin
	$condition = $inlineedit==EDIT_INLINE;//($inlineedit) inline mode

	if($condition)
	{
		$value = postvalue("value_respondido_".$id);
		$type = postvalue("type_respondido_".$id);
		if(FieldSubmitted("respondido_".$id))
		{
				$value = prepare_for_db("respondido",$value,$type);
		}
		else
			$value = false;
		...
		{	
			...
			if(0 && "respondido"=="senha" && $url_page=="admin_users_")
				$value = md5($value);
			$evalues["respondido"] = $value;
		
			}
	
		}
//	processing respondido - end
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: orcamento_".$pageObject->getPageType().".php?".$keyGetQ);
...
/////////////////////////////////////////////////////////////
$query = $queryData_orcamento->Copy();
...
if(!$data)
{
	...
	{
		header("Location: orcamento_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
if($readevalues)
{
	$data["respondido"] = $evalues["respondido"];
...
//	endereco - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"endereco", ""),"field=endereco".$keylink,"",MODE_LIST);
...
//	cidade - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"cidade", ""),"field=cidade".$keylink,"",MODE_LIST);
...
//	estado - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"estado", ""),"field=estado".$keylink,"",MODE_LIST);
...
//	receita - Document Download
	...
	$showValues["receita"] = $value;
	$showFields[] = "receita";
		$showRawValues["receita"] = substr($data["receita"],0,100);

//	mensagem - 

		$value = "";
			$value = ProcessLargeText(GetData($data,"mensagem", ""),"field=mensagem".$keylink,"",MODE_LIST);
	$showValues["mensagem"] = $value;
	$showFields[] = "mensagem";
		$showRawValues["mensagem"] = substr($data["mensagem"],0,100);

//	respondido - Checkbox

		$value = "";
			$value = GetData($data,"respondido", "Checkbox");
	$showValues["respondido"] = $value;
	$showFields[] = "respondido";
		$showRawValues["respondido"] = substr($data["respondido"],0,100);
