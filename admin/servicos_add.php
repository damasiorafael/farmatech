...
if($inlineadd == ADD_INLINE)
	$templatefile = "servicos_inline_add.htm";
else
	$templatefile = "servicos_add.htm";
...
//	processing titulo - start
	...
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["titulo"]=$value;
		}
		}
//	processibng titulo - end
//	processing img_destaque - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		...
		if (FieldSubmitted("img_destaque_".$id))
		{
				$fileNameForPrepareFunc = postvalue("filename_img_destaque_".$id);
					$value=prepare_upload("img_destaque","upload2",$fileNameForPrepareFunc,$fileNameForPrepareFunc,"" ,$id,$pageObject);
		}
		else
			$value=false;
		...
		//	create thumbnail
			if($value)
			{
				...
				$value=md5($value);
			$avalues["img_destaque"]=$value;
		}
		}
//	processibng img_destaque - end
//	processing img_interna - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		...
		if (FieldSubmitted("img_interna_".$id))
		{
				$fileNameForPrepareFunc = postvalue("filename_img_interna_".$id);
					$value=prepare_upload("img_interna","upload2",$fileNameForPrepareFunc,$fileNameForPrepareFunc,"" ,$id,$pageObject);
		}
		else
			$value=false;
		...
		//	create thumbnail
			if($value)
			{
				...
				$value=md5($value);
			$avalues["img_interna"]=$value;
		}
		}
//	processibng img_interna - end
...
//	processing destaque_home - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		...
		if(!($value===false))
		{
				...
				$value=md5($value);
			$avalues["destaque_home"]=$value;
		}
		}
//	processibng destaque_home - end
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'servicos_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'servicos_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: servicos_".$pageObject->getPageType().".php");
