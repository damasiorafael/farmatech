...
$templatefile = ($inlineedit == EDIT_INLINE) ? "contato_inline_edit.htm" : "contato_edit.htm";
...
if ($IsSaved && no_output_done() && $inlineedit == EDIT_SIMPLE)
{
	...
	// redirect
	header("Location: contato_".$pageObject->getPageType().".php?".$keyGetQ);
...
if(!$data)
{
	...
	{
		header("Location: contato_list.php?a=return");
		exit();
	}
	else
		$data = array();
...
//	assunto - 
		...
		$value = "";
			$value = ProcessLargeText(GetData($data,"assunto", ""),"field=assunto".$keylink,"",MODE_LIST);
	$showValues["assunto"] = $value;
	$showFields[] = "assunto";
		$showRawValues["assunto"] = substr($data["assunto"],0,100);
