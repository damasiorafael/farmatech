...
function checkTableName($shortTName, $type=false)
{
	...
	if ("banner_home" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	...
	if ("servicos" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	...
	if ("laboratorio" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	if ("orcamento" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
	...
	if ("area_restrita_users" == $shortTName && ($type===false || ($type!==false && $type == 0)))
	{
		return true;	
	}
...
{
	$arr=array();
	$strPerm = GetUserPermissions("banner_home");
	...
	$strPerm = GetUserPermissions("servicos");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="servicos";
	}
	...
	$strPerm = GetUserPermissions("orcamento");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="orcamento";
	}
	...
	$strPerm = GetUserPermissions("area-restrita-users");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="area-restrita-users";
	}
	$strPerm = GetUserPermissions("area-restrita");
	if(strpos($strPerm, "P")!==false || ($pdfMode && strpos($strPerm, "S")!==false))
	{
		$arr[]="area-restrita";
	}
...
function GetTableURL($table="")
{
	...
	if("empresa"==$table) 
		return "empresa";
	if("laboratorio"==$table) 
		return "laboratorio";
	if("orcamento"==$table) 
		return "orcamento";
	if("contato"==$table) 
		return "contato";
	if("area-restrita-users"==$table) 
		return "area_restrita_users";
	if("area-restrita"==$table) 
...
{	
	...
	if("users"==$shortTName) 
		return "users";
	if("servicos"==$shortTName) 
		return "servicos";
	if("empresa"==$shortTName) 
		return "empresa";
	if("laboratorio"==$shortTName) 
		return "laboratorio";
	if("orcamento"==$shortTName) 
		return "orcamento";
	if("contato"==$shortTName) 
		return "contato";
	if("area_restrita_users"==$shortTName) 
		return "area-restrita-users";
...
// long binary data?
	...
	else if($format == FORMAT_FILE_IMAGE)
	{
		...
		$thumbnailed=false;
		$thumbprefix="";
		if($strTableName=="banner_home" && $field=="imagem")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="servicos" && $field=="img_destaque")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="servicos" && $field=="img_interna")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="empresa" && $field=="foto")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
		if($strTableName=="laboratorio" && $field=="foto")
		{
			$thumbnailed=true;
			$thumbprefix="th";
		}
...
function ProcessLargeText($strValue, $iquery="",$table="", $mode=MODE_LIST, $format = "")
{
	...
	if($format!=EDIT_FORMAT_LOOKUP_WIZARD && $cNumberOfChars>0 && strlen($strValue)>$cNumberOfChars && $mode==MODE_LIST)
	{
		...
		$ret.=' <a href="javascript:void(0);" query="fulltext.php?table='.$table.'&'.$iquery.'">'."Mais".' ...</a>';
	}
...
function ReadUserPermissions($userID="")
{
	...
	if($userID!="Guest")
	{
		$sql = "select GroupID from `farmatech_ugmembers` where UserName='".db_addslashes($userID)."'";
	...
	$rights=array();
	$sql = "select TableName,AccessMask from `farmatech_ugrights` where GroupID in (".$groupstr.")";
	$rs = db_query($sql,$conn);
	while($data=db_fetch_numarray($rs))
	{
...
{	
	...
	if(!count($_SESSION["UserRights"]["Guest"]))
		return false;
	if(array_key_exists("banner_home",$_SESSION["UserRights"]["Guest"]))
		return true;
	...
	if(array_key_exists("users",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("servicos",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("empresa",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("laboratorio",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("orcamento",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("contato",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("area-restrita-users",$_SESSION["UserRights"]["Guest"]))
		return true;
	if(array_key_exists("area-restrita",$_SESSION["UserRights"]["Guest"]))
		return true;
	return false;
}
...
// ajax	dependent dropdown
			...
			{	
				...
				echo "&nbsp;<a href=# id=".$openlookup.">"."Selecione"."</a>";				
			}
			echo '<input type="hidden" id="'.$cfield.'" name="'.$cfield.'">';
//	add new item link
			if($addnewitem)
			{
				echo "&nbsp;<a href=# id='addnew_".$cfield."'>"."Adicionar Novo"."</a>";
			}
			return;
		}
////////////////////////////////////////////////////////////////////////////////////////////
...
//	filename edit
			...
			if ( $edit==MODE_INLINE_EDIT ) {
				$strtype='<br><input id="'.$ctype.'_keep" type="Radio" name="'.$ctype.'" value="upload0" checked>'."Manter";
			} else {
				$strtype='<br><input id="'.$ctype.'_keep" type="Radio" name="'.$ctype.'" value="upload0" checked>'."Manter";
			}
