...
this.showErrorHappend(html);return;}
...
$(loadedContent).empty();}
this.hideErrorHappend();$(gBlock_old).empty();if(html)
...
{var mes=this.getMessageDiv();if(!mes)
return false;$(mes).empty();$(mes).addClass('mes_not');$(mes).css('text-align','left');createInlineError(this.prm.pageId,mes,txt)
window.scrollTo(0,0);$(mes).show();}
this.hideErrorHappend=function()
{var mes=$("#usermessage"+this.prm.pageId);if($(mes).length&&$(mes).css('display')!='none')
{$("#usermessage"+this.prm.pageId).fadeOut(1000);$("#usermessage"+this.prm.pageId).empty();}}
