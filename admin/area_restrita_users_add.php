...
if($inlineadd == ADD_INLINE)
	$templatefile = "area_restrita_users_inline_add.htm";
else
	$templatefile = "area_restrita_users_add.htm";
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'area_restrita_users_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'area_restrita_users_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: area_restrita_users_".$pageObject->getPageType().".php");
...
if(@$_POST["a"]=="added" && ($inlineadd == ADD_INLINE || $inlineadd == ADD_MASTER || $inlineadd==ADD_POPUP)) 
{
	...
	$masterquery="mastertable=area%2Drestrita%2Dusers";
	$masterquery.="&masterkey1=".rawurlencode($data["id"]);
	$showDetailKeys["area_restrita"]=$masterquery;
...
//	cep - 
	...
	{	
		$value="";
			$value = ProcessLargeText(GetData($data,"cep", ""),"field=cep".$keylink,"",MODE_LIST);
