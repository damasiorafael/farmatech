...
if($inlineadd == ADD_INLINE)
	$templatefile = "area_restrita_inline_add.htm";
else
	$templatefile = "area_restrita_add.htm";
	...
	if($inlineAddOption)
	{
		...
		if (FieldSubmitted("arquivo_".$id))
		{
				$fileNameForPrepareFunc = postvalue("filename_arquivo_".$id);
					$value=prepare_upload("arquivo","upload2",$fileNameForPrepareFunc,$fileNameForPrepareFunc,"" ,$id,$pageObject);
		}
		else
			$value=false;
		...
		{
				if($value)
				$contents = GetUploadedFileContents("value_arquivo_".$id);

		//	create thumbnail
			if($value)
			{
						$ext = CheckImageExtension(GetUploadedFileName("value_arquivo_".$id));
				$thumb = CreateThumbnail($contents,150,$ext);
				$pageObject->filesToSave[] = new SaveFile($thumb,"th".$value,GetUploadFolder("arquivo"), GetFieldData("","arquivo","Absolute",false));
			}

						if(0 && "arquivo"=="senha" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["arquivo"]=$value;
		}
		}
//	processibng arquivo - end
//	processing exclusivo - start
	$inlineAddOption = true;
	$inlineAddOption = $inlineadd==ADD_INLINE;
	if($inlineAddOption)
	{
		$value = postvalue("value_exclusivo_".$id);
		$type=postvalue("type_exclusivo_".$id);
		if (FieldSubmitted("exclusivo_".$id))
		{
				$value=prepare_for_db("exclusivo",$value,$type);
		}
		else
			$value=false;
		...
		if(!($value===false))
		{
						...
						if(0 && "exclusivo"=="senha" && $url_page=="admin_users_")
				$value=md5($value);
			$avalues["exclusivo"]=$value;
		}
		}
//	processibng exclusivo - end
...
//	insert masterkey value if exists and if not specified
	if(@$_SESSION[$sessionPrefix."_mastertable"]=="area-restrita-users")
	{
		if(postvalue("masterkey1"))
			$_SESSION[$sessionPrefix."_masterkey1"] = postvalue("masterkey1");
		
		if($avalues["usuario"]=="")
			$avalues["usuario"]=prepare_for_db("usuario",$_SESSION[$sessionPrefix."_masterkey1"]);
			...
			if($inlineadd==ADD_SIMPLE || $inlineadd==ADD_MASTER)
			{
				...
				if (count($keys))
				{
					...
					if(GetTableData($strTableName,".edit",false) && $permis['edit'])
						$message .='&nbsp;<a href=\'area_restrita_edit.php?'.$keylink.'\'>'."Editar".'</a>&nbsp;';
					if(GetTableData($strTableName,".view",false) && $permis['search'])
						$message .='&nbsp;<a href=\'area_restrita_view.php?'.$keylink.'\'>'."Exibir".'</a>&nbsp;';
				}
				$mesClass = "mes_ok";	
			}
		}
...
if (no_output_done() && $inlineadd==ADD_SIMPLE && $IsSaved)
{
	...
	// redirect
	header("Location: area_restrita_".$pageObject->getPageType().".php");
...
if(@$_SESSION[$sessionPrefix."_mastertable"]=="area-restrita-users")
{
	...
	$defvalues["usuario"] = @$_SESSION[$sessionPrefix."_masterkey1"];	
...
//	arquivo - Document Download
	...
	$showValues["arquivo"] = $value;
	$showFields[] = "arquivo";
		$showRawValues["arquivo"] = substr($data["arquivo"],0,100);
	}	
//	exclusivo - Checkbox
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
	...
	$showValues["exclusivo"] = $value;
	$showFields[] = "exclusivo";
		$showRawValues["exclusivo"] = substr($data["exclusivo"],0,100);
	}	
//	usuario - 
	$display = false;
	if($inlineadd==ADD_INLINE || $inlineadd==ADD_ONTHEFLY || $inlineadd==ADD_POPUP)
	...
	{	
		$value="";
				$value = DisplayLookupWizard("usuario",$data["usuario"],$data,$keylink,MODE_LIST);
